...
var _inpFocusFnc = {
	init : function($parentTgt) {
		...
		$('.inp-txt, .inp-pw, .inp-search, .inp-unit, .inp-date', $parentTgt).off('click.share').on('click.share', function(e) {//�명뭼�띿뒪�� �꾨땶 �곸뿭 �대┃�� ��젣 踰꾪듉 hide泥섎━
		...
		$('.inp-txt input, .inp-pw input, .inp-search input, .inp-unit input, .inp-date input', $parentTgt).off('change.share keyup.share paste.share focusin.share').on('change.share keyup.share paste.share focusin.share', function() {
			...
			if(!$del) {
				$this.after('<button type="button" class="del"><span class="offscreen">��젣</span></button>');
				$del = $this.next();
...
var _tableScrollFnc = {
	...
	fixeScroll : function(target) {
		...
		if($('.fixed-item', $tableBox).length > 0) {//scroll x �뗮똿
				...
				if($conts.is(':hidden')) {
					...
					$wrap.each(function(){
						if($wrap.find('.btn-expand').hasClass('foldup') == false){
							...
							$('.btn-acco-full', $accordianWrap).find('span').text('紐⑤몢�リ린');
						}
					});
				}else{
					$this.addClass('foldup');
					...
					$('.btn-acco-full', $accordianWrap).find('span').text('紐⑤몢�닿린');
				}
			}
		});
	},
	...
	accoAllAction : function(target){
		...
		$btnAccoFull.off('click.share').on('click.share', function() {
			if($btnAccoFull.hasClass('allAct')){
				...
				$btnAccoFull.find('span').text('紐⑤몢�닿린');
				$('.btn-expand', $titBox).addClass('foldup');
				$constBox.hide().attr('aria-hidden','true');
			}else{
				$btnAccoFull.addClass('allAct');
				$btnAccoFull.find('span').text('紐⑤몢�リ린');
				$('.btn-expand', $titBox).removeClass('foldup');
...
var _tabBoxFnc = {
	...
	moreDefault : function(target) {
		...
		var html = '<button type="button" class="btn-tab-more" aria-expanded="false"><em class="offscreen">�� �붾낫湲�</em></button>';
...
var _scrollHideShowFnc = {
	...
	winScrollEvent : function() {
		...
		var $searchFixed = $('.search-form-wrap .inner-box.fixed').length ? $('.search-form-wrap .inner-box.fixed') : false;//寃��됱쁺��
		var $fundCateSlide = $('.recommend-fund-wrap .fund-category-box').length ? $('.recommend-fund-wrap .fund-category-box') : false;//寃��됱쁺��
		var $topFixed = $('.top-fixed-area .inner-box.fixed').length ? $('.top-fixed-area .inner-box.fixed') : false; //�곷떒 怨좎젙 �곸뿭
	...
	layerScrollEvent : function(target) {
		...
		var $compareChPoint = $compareSelectBox.length ? $compareSelectBox.outerHeight() : undefined;//鍮꾧탳�섍린 �앹뾽 �ㅻ뜑遺�遺� �몄텧/鍮꾨끂異� �곸슜 �꾩튂 蹂�寃�
		var $fundDetailInfoWrap = $('.fund-detail-info-wrap.type-sticky');
		var $fundDetailInfoChPoint = $fundDetailInfoWrap.length ? $fundDetailInfoWrap.outerHeight() : undefined;//���쒖긽�몄젙蹂� �앹뾽 �ㅻ뜑遺�遺� �몄텧/鍮꾨끂異� �곸슜 �꾩튂 蹂�寃�

		var $scrollfnc = function(e) {
			var $scrollST = $(e.target).scrollTop();
			var $isScrollUp = $scrollST - $nowScroll < 0 ? true : false;

			if($compareSelectBox.length) {//鍮꾧탳�섍린
			...
			}else if($fundDetailInfoWrap.length) {//���쒖긽�몄젙蹂� sticky
...
var _swiperFnc = {
	...
	setSwiper : function(target) {
		...
		if($swiperSlide.length == 1){//slide 媛쒖닔媛� �쒓컻�쇰븣 
			var $baseOpt = {
				autoplay: false,
				loop: false,
				resistanceRatio : 0,
			...
			var $baseOpt = {
				...
				observeParents: true,
				a11y: {
					paginationBulletMessage: '{{index}}踰� �щ씪�대뱶 �대룞',
					prevSlideMessage: '�댁쟾 �щ씪�대뱶 �대룞',
					nextSlideMessage: '�ㅼ쓬 �щ씪�대뱶 �대룞'
				},
			}
		}
		...
		if($wrap.data('autoplay') == true) { /* autoplay */
			$swiperOpt.autoplay = {}
			// autoplay-delay 媛믪씠 0�� ��, false濡� �몄떇�섎뒗 �ㅻ쪟
			// 議곌굔媛� �몃텇��
...
var _uiRangeSlider = {
	init: function($parentTgt){
		...
		var rangeObj = this;
		if(!$('.range-slider-case .range-slider-bar', $parentTgt).length) return; //range slide 占쎈뗄�� 占쎈냱�ｏ옙占� 占쎄쑬�� 占쎈슣�� 占썬끋六억옙�륅옙 占쎈봿��
	...
	setRange : function(target) {
		...
		$('.range-slider', $target).slider({
			...
			slide: function(event, ui) {//callback
				// console.log('min value', ui.values[0]);//占쎄쑴�� 筌ㅼ뮇�쇔첎占�
				// console.log('max value', ui.values[1]); //占쎄쑴�� 筌ㅼ뮆占썲첎占�
			}
		});
...
/* loading
	�ㅽ뻾 : loading('open') / �リ린 : loading('close') 
*/
...
$(document).ready(function() {
	...
	FontFaceOnload("SpoqaHanSansNeo", {
		success: function() {
			...
			_gnbFnc.init();//GNB
			_globalSearchFnc.init();//�듯빀寃���
			_subMenuSelect.init();//�쒕툕硫붾돱 由ъ뒪�� �앹뾽
			_goToTopFnc.init();//�묒쑝濡� 媛�湲곕쾭��
			_inpFocusFnc.init();//input focus ��젣 踰꾪듉 �몄텧
			_calendarFnc.init();//�щ젰
			_tableScrollFnc.init();//�뚯씠釉� 怨좎젙 �ㅽ겕濡�
			_accordianFnc.init();//�꾩퐫�붿뼵 諛뺤뒪
			_toggleBtnFnc.init();//�좉�踰꾪듉
			_scrapToggleFnc.init();//scrap 踰꾪듉
			_boardDetailFnc.init();//寃뚯떆�� �곸꽭 �섏젙��젣 �앹뾽
			_popupFnc.init();//�앹뾽
			_footerSwiper.init();//footer swiper 
			_stickyBtnFnc.init();//sticky button box 
			_tabBoxFnc.init();//tab box
			_compareFloatFoldFnc.init();//湲덉쑖�곹뭹 鍮꾧탳�섍린 sticky bar
			_foldItemCaseFnc.init();//�대뵫諛뺤뒪 �묓옒/�쇱묠 湲곕뒫
			_uiRangeSlider.init();//ui range slide
			_compareSelectFnc.init();//鍮꾧탳�섍린 select
			_checkInfomationFnc.init();//�뺤씤�섏꽭��
			_scrollHideShowFnc.init();//header scroll 諛⑺뼢�� �곕Ⅸ �몄텧�щ�
			_swiperFnc.init();//swiper
			_swiperCarouselFnc.init();//swiper-Carousel ����(醫뚯슦踰꾪듉)
			_swiperGalleryFnc.init();//gallery swiper 援�궡�ъ옄 �쒖옉�섍린 
			_imgScrollFnc.init();//img 媛�濡� �ㅽ겕濡�
			_consumerGnbFnc.init();//�뚮퉬�먮낫�명룷�� �꾩껜硫붾돱
			_etcFnc.init(); //湲고�
		},
		error: function() {
