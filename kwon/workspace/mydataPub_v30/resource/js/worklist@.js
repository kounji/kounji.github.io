$(document).ready(function(){
	...
	function gridTab(){
		...
		$('#tab-menu').append("<li><a href='webPublish/guide/guide.html' target='_blank'>퍼블 Guide</a></li>");//가이드
...
//			console.log(count +"--"+ count2);
			...
			$('.stage').find('[aria-label=count-total]').text(count2 +' / '+ arr2);
		});
		var per = (count2 * 100) / arr2;
		per = per.toString();
		var per2 = per.split('.');
		$('.stage').find('[aria-label=count-rate]').text(per2[0] + ' / 100');
