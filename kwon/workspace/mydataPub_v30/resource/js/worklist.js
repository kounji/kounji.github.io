...
function scrollTab(){
	...
	$menu.on('click','a',function(e){
		...
		var section = $conts.eq(idx);
		var offsetTop = section.offset().top - $(".header").outerHeight() + 1;
		$doc.stop().animate({
			scrollTop : offsetTop
		},300);
		return false;
	...
	$(window).scroll(function(){
		...
		$.each($conts, function(idx,item){
			var $target = $conts.eq(idx);
			var i = $target.index();
			var targetTop = $target.offset().top -  $(".header").outerHeight() + 1;
			//console.log(scltop)
			if(targetTop <= scltop){
				$menu.removeClass('on');
...
$(document).ready(function(){
	...
	$('.cont').each(function(i){
		scrollTop[i] = $('.cont').eq(i).position().top - 100;
	...
	$(window).scroll(function(){
		for(var i=0;i<=scrollTop.length;i++){
			if($(this).scrollTop() > scrollTop[i]){
				$('.menu li').eq(i).addClass('on').siblings('li').removeClass('on');
			}
		}
	...
	$('.gnb .menu li').on('click', function(e){
		...
		//var scroll = target == '#list00' ? 0 : $(target).position().top + 200;
		//$('html, body').scrollTop(scroll);
	})
*/
	$('body').click(function(e){
		if(!$(e.target).is('.btn-preview, .link, .menu')){
	...
	$('.btnContinue').click(function(){
		var _this = jQuery(this);
		if(_this.parent().find('#page tr.continue').length != 0){//해당 항목에 유지열이 있으면 실행
			if(_this.hasClass('dn')){
		...
		if(jQuery('#wrap').find('.cont > .dn').length >= 1){//유지 항목이 1개 이상 열리면 일괄닫기
			$('.btnContinue.all').addClass('dn').text('유지 일괄 접기');
		}else{
			$('.btnContinue.all').removeClass('dn').text('유지 일괄 펼침');
		};
	});
		...
		var ColorBox = (function(){
			...
			var colorBoxHandler = function(){
				...
				function addTitle(){
					...
					var curMenu = $('.menu').find('li').eq(curDepNum).text();
	...
	function insertList(menuid,depth1,depth2,depth3,depth4,name,id,type,wtype, opencont,writer,date,comment){
		...
		if(menuid.indexOf('list') != -1){
			...
			// }else{
				...
				//[v4.0]오픈컨텐츠(Y컨텐츠 있음, 공백은 오픈컨텐츠 없음)
				if(opencont == 'Y'){
					opencont = "<i class='check'></i> O"
				}

				//alert(disabled);

				//[v4.0]화면아이디 프리뷰
				cell = "<a href='webPublish/html/"+code+"/"+id+".html' class='link' target='_blank'>"+id+"</a>";
				/*
				//상태 변환(M작업,Y완료)
				if(status == 'Y'){	//완료
					if(id.lastIndexOf(".vw") > -1 || id.lastIndexOf(".do") > -1){
						...
						cell = "<a href='webPublish/html/"+code+"/"+id+".html' class='link' target='_blank'>"+id+"</a><a rel='iframe' href=\ webPublish/html/"+code+"/"+id+".html\ class='iframe cboxElement btn-preview'>미리보기</button>";
			...
			html = "<tr class="+disabled+">" + 
				...
				"<td>"+name+"</td>" + //화면명
				"<td>"+cell+"</td>" + //화면ID
				"<td class='center' data-element='"+type+"'>"+type+"</td>" + //유형
				"<td class='center'>"+wtype+"</td>" + //작업구분
				"<td class='center'>"+opencont+"</td>" + //오픈컨텐츠
	...
	function gridTab(){
		...
		$('#tab-menu').append("<li><a href='old_2023/index.html' target='_blank'>2023 퍼블</a></li>");//2023 화면
	...
	function insertHeader(){
		...
		$("#wrap .cont").each(function(){
			$("> table", $(this)).append($(colgroup), $(thead));
			$("> #page table", $(this)).append(colgroup);
		});
	}
...
$(document).ready(function(){
	...
	$('.cont').each(function(){
		if($(this).find('tr').hasClass('continue')) {
			$(this).find('h2').append('<button class="btn-continue"><span>유지</button>')
		}
	});
