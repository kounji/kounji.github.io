<!--
/*************************************************************************
* @ 서비스경로 : 큰글모드 > 지출 > 현금이용금액
* @ 페이지설명 : 큰글모드 > 지출 > 현금이용금액
* @ 파일명     : src/views/page/LC/TA/LCTA4S06/LCTA4S06.vue
* @ 작성자     : CS541597
* @ 작성일     : 2025-02-20
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
            ...
            <div aria-hidden="false" class="slide_popup">
                ...
                <div class="popup_content">
                    <div class="wallet_senior">
                        <div class="gray_box">
                            <div class="analysis" :class="comparedChsvSam > 0 ? 'up' : comparedChsvSam < 0 ? 'down' : ''">
                                <p v-html="getComparedStr(comparedChsvSam)"></p>
                            </div>
                        </div>

                        <div class="board_box">
                            <ul class="lately_list">
                                <li v-for="(item, idx) in chsvList" :key="idx">
                                    <a href="javascript:void(0);" @click.prevent="openPayDetail(item)">
                                        <i class="icon_money"></i>
                                        <div>
                                            <span v-if="item.chsvKdnm == '이체'" class="name">계좌 이체로 쓴 금액</span>
                                            <span v-if="item.chsvKdnm == '현금'" class="name">직접 등록한 지출 금액</span>
                                        </div>
                                        <span class="num"><em>{{item.tram | numberFilter}}</em>원</span>
                                    </a>
                                </li>
                            </ul>
                        </div>
...
import LCLE4104 from '@/views/page/LC/LE/LCLE4104/LCLE4104.vue'
...
export default {
    ...
    data: () => {
        return {
            chsvList        : [],   // 현금 이용금액 목록
            schXpsSam       : 0,    // 선택 당월 현금이용금액
            bmmSchXpsSam    : 0,    // 선택 전월 현금금액
    ...
    methods: {
         ...
         */
        getData(param) {
            this.chsvList = param.trgtList || []
            this.schXpsSam = param.nowSam || 0
            this.bmmSchXpsSam = param.bfSam || 0
