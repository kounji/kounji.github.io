<!--
/*************************************************************************
* @ 서비스경로 : 하단메뉴 > 지출분석 > 금융달력 > 수입/지출 추가
* @ 페이지설명 : 하단메뉴 > 지출분석 > 금융달력 > 수입/지출 추가
* @ 파일명     : src/views/page/LC/FD/LCFD1001/LCFD1001.vue
* @ 작성자     : CS515901
* @ 작성일     : 2021-06-09
************************** 수정이력 ****************************************
*	날짜                    작업자					변경내용
...
*   2022-08-23              CS528061                고도화 및 파일명 변경(LCFD1106 -> LCFD1001)
*************************************************************************/
-->
<template>
	<!-- full popup S -->
		...
		<div class="popup_content"><!-- 하단 버튼 없을때 com_no_bottom -->
			<div class="com_inner">
				<div class="com_input_type01 com_word1">
					<input type="tel" id="LC-FD-01-01" class="input_right" :class="tram.length > 0 ? 'focusON focusforce ' : ''" required="" maxlength="17" placeholder="금액을 입력해 주세요." 
                            title="수입 금액 입력해주세요." v-model="tram" @keyup="addComma(), tram.length>0 ? moveNextTag($event):''" @input="fn_setVisible('sec1')" ref="tram">
				...
				<div class="com_input_type01" v-show="vis_sec2">
                    ...
                    <input type="text" id="LC-FD-06-02" v-else class="" :class="trCntn.length > 0 ? 'focusON focusforce' : ''" name="trCntn" :value="trCntn" 
                            required="" placeholder="지출 내용을 입력하세요." title="수입 내용을 입력하세요." maxlength="40" @keyup="moveNextTag($event), fn_checkWord($event, 40, 'trCntn'), fn_setVisible('sec3')" ref="trCntn">
				...
				<!-- 날짜 -->
				<div class="com_input_type01 com_word2" role="button" v-show="vis_sec3">
					<input type="text" id="LC-FD-06-03" data-popup="alert_popup" readonly class="input_cal_date com_txtright_type01 tar" :class="trDtm.length > 0 ? 'focusON focusforce' : ''" 
                            required="" data-placeholder="날짜를 선택하세요." title="날짜를 선택하세요." aria-hidden="true" v-model="trDtm" @click="fn_setVisible('sec4'), fn_popupCalendar($event)">
					...
					<dl class="com_line">
						...
						<dd>
							<span class="com_btn_switch">
								<input type="checkbox" id="check02" @click="myNongAdd()" ref="my_check01">
