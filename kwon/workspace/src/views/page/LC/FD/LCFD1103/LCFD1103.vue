<!--
/*************************************************************************
* @ 서비스경로 : 하단메뉴 > 지출분석 > 금융달력 > 수입/지출/이체 상세내역
* @ 페이지설명 : 하단메뉴 > 지출분석 > 금융달력 > 수입/지출/이체 상세내역
* @ 파일명     : src/views/page/LC/FD/LCFD1103/LCFD1103.vue
* @ 작성자     : CS515901
* @ 작성일     : 2021-05-10
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
        ...
        <div class="popup_content com_bg_type00">
				...
				<div class="new_top_box type01" v-if="asetCtgrDsc === '1'">
					...
					<div class="box_content">
						<div class="price">
							...
							<div class="period"><span>{{fn_date(trDt, 'YYYY.MM.DD')}}</span></div>
						</div>
					</div>             
				</div>
				<!-- 지출 -->
				<div class="new_top_box type01" v-if="asetCtgrDsc === '2'">
					...
					<div class="box_content">
						<div class="price">
							...
							<div class="won"><span>{{tram | numberFilter}}</span><span>원</span>
							</div>
							<div class="period"><span>{{fn_date(trDt, 'YYYY.MM.DD')}} </span><span v-if="cdPpayStlStsc==='03'">{{cdPpayStlStsCnm}}</span></div>
						</div>
					</div>             
				</div>
				<!--이체-->
			...
			<div class="com_inner com_bg_type00 com_inputarea_type07" v-if="asetCtgrDsc !== '3' || inShowFlag || outShowFlag">
				<template v-if="asetCtgrDsc === '1' || inShowFlag">
					<div class="com_btnselectbox_type01">
						...
						<button type="button" class="com_btnselect_type01" @click="selectCategory('1')"><span>{{asetAmnCtgrId == '' ? '수입카테고리 선택' : ctgrnm}}</span></button>
					</div>
					<div class="com_input_type01 ani-active" v-if="(prcMchtnm.length>0 && dataSrcDsc !== '05') || dataSrcDsc === '05' || dpzAcTrTpc === '03'">
						<input type="text" id="com_input01" class="" name="" :value="prcMchtnm" required placeholder="수입 내용을 입력하세요."
						maxlength="40" :disabled="dataSrcDsc !=='05'" :class="prcMchtnm.length>0?'focusON focusforce':''" @keyup="fn_checkWord($event, 40, 'prcMchtnm')" ref="prcMchtnm">
						<label for="com_input01"><em v-if="dataSrcDsc === '05'"><span class="blind">필수입력</span></em><span class="txt_label_x">수입 내용</span></label>
						<!-- <label for="com_input01" class="necessary"><em><span class="blind">필수입력</span></em></label>
						<input type="text" id="com_input01" class="" name="" value="" required placeholder="수입항목을 입력하세요" v-model="prcMchtnm" maxlength="100" :disabled="dataSrcDsc==='02' || dataSrcDsc==='03'"> -->
					</div>
					<div class="com_textarea_type01 ani-active">
					...
					<!-- <textarea name="" id="" class="com_txtarea_type01" cols="10" rows="5" placeholder="메모를 입력하세요 (선택)" v-model="memoCntn" maxlength="100">
					</textarea> -->
				</template>
				<template v-if="asetCtgrDsc === '2' || outShowFlag">
					<div class="com_btnselectbox_type01">
						<span class="com_iconstar"><span class="blind">필수입력</span><em>카테고리</em></span>
						<button type="button" class="com_btnselect_type01" @click="selectCategory('2')"><span>{{asetAmnCtgrId == '' ? '지출카테고리 선택' : ctgrnm}}</span></button>
						<!-- <button type="button" class="com_btn_select com_iconstar" @click="selectCategory('2')"><em class="blind">필수입력</em><span>{{asetAmnCtgrId == '' ? '지출카테고리 선택' : ctgrnm}}</span></button> -->
					</div>
					<div class="com_input_type01 ani-active" v-if="(prcMchtnm.length>0 && dataSrcDsc !== '05') || dataSrcDsc === '05' || dpzAcTrTpc === '02'">
						<input type="text" id="com_input02" class="" name="" :value="prcMchtnm" required placeholder="지출 내용을 입력하세요." 
						maxlength="40" :disabled="dataSrcDsc !== '05'" :class="prcMchtnm.length>0?'focusON focusforce':''" @keyup="fn_checkWord($event, 40, 'prcMchtnm')" ref="prcMchtnm">
						<label for="com_input02"><em v-if="dataSrcDsc === '05'"><span class="blind">필수입력</span></em><span class="txt_label_x">지출 내용</span></label>
						<!-- <label for="com_input02" class="necessary"><em><span class="blind">필수입력</span></em></label>
						<input type="text" id="com_input02" class="" name="" value="" required placeholder="지출항목을 입력하세요" v-model="prcMchtnm" maxlength="100" :disabled="dataSrcDsc==='02' || dataSrcDsc==='03'"> -->
					</div>
					<div class="com_textarea_type01 ani-active">
						<textarea name="" id="txt00" class="com_txtarea_type01" cols="10" rows="5" required placeholder="메모를 입력하세요" :value="memoCntn" maxlength="100" @keyup="fn_checkWord($event, 100, 'memoCntn')" ref="memoCntn"></textarea>
						<label for="txt00"><span class="txt_label_x">메모</span></label>
			...
			<div class="com_inner com_bg_type00" v-show="asetCtgrDsc !== '3' || showFlag">
				<div class="com_setlist_type01 ani-active">
					...
					<dl v-show="asetCtgrDsc === '1' || inShowFlag">
						<dt>
							<p>마이농가 수입에 추가</p>
						...
						<dd>
							<span class="com_btn_switch">
								<input type="checkbox" id="my_check01" @click="myNongAdd('1')" ref="my_check01" :checked="myFhsYn==='1'">
								<label for="my_check01">
									<span class="txt">선택</span>
									<span role="img" class="on">선택</span>
									<span role="img" class="off">해제</span>
								</label>
					...
					<dl v-show="asetCtgrDsc === '1' && dpzAcTrTpc === '03'">
						<dt>이체→수입내역으로 이동하기</dt>
						<dd>
							<span class="com_btn_switch">
								<input type="checkbox" id="imcomeMove" :checked="dpzAcTrTpc==='03'?true:false" ref="imcomeMove">
								<label for="imcomeMove">
									<span class="txt">선택</span>
									<span role="img" class="on">선택</span>
									<span role="img" class="off">해제</span>
								</label>
					...
					<dl v-show="asetCtgrDsc === '2'">
						...
						<dd>
							<span class="com_btn_switch">
								<input type="checkbox" id="hideMinus" @click="hideMinus()" ref="hideMinus" :checked="xpsBrkExpsYn==='1'?false:true" >
								<label for="hideMinus">
									<span class="txt">선택</span>
									<span role="img" class="on">선택</span>
									<span role="img" class="off">해제</span>
					...
					<dl v-show="asetCtgrDsc === '2'">
						...
						<dd>
							<span class="com_btn_switch">
								<input type="checkbox" id="deleteMinus" :checked="bgtIncdYn === '1'?false:true" ref="deleteMinus">
								<label for="deleteMinus">
									<span class="txt">선택</span>
									<span role="img" class="on">선택</span>
									<span role="img" class="off">해제</span>
								</label>
					...
					<dl v-show="asetCtgrDsc === '2' && dpzAcTrTpc === '02'">
						<dt>이체→지출내역으로 이동하기</dt>
						<dd>
							<span class="com_btn_switch">
								<input type="checkbox" id="minusMove" :checked="dpzAcTrTpc==='02'?true:false" ref="minusMove">
								<label for="minusMove">
									<span class="txt">선택</span>
									<span role="img" class="on">선택</span>
									<span role="img" class="off">해제</span>
					...
					<dl v-show="asetCtgrDsc === '2' || outShowFlag">
						...
						<dd>
							<span class="com_btn_switch">
								<input type="checkbox" id="myMinusAdd" @click="myNongAdd('2')" ref="my_check02" :checked="myFhsYn==='1'">
								<label for="myMinusAdd">
									<span class="txt">선택</span>
									<span role="img" class="on">선택</span>
									<span role="img" class="off">해제</span>
								</label>
...
import LCFD1104 from '@/views/page/LC/FD/LCFD1104/LCFD1104'
import LCFD1107 from '@/views/page/LC/FD/LCFD1107/LCFD1107'
...
export default {
    ...
    methods: {
        ...
        getData() {
			console.log('LCFD1103', this.modalConfig.param)
			...
			console.log(JSON.parse(this.p_inqKey))
		...
		selectCategory(flag){
			...
			modalService.openSlidePagePopup(config).then(response => {
				...
				this.asetAmnCtgrId = response.asetAmnCtgrId
				this.ctgrnm = response.ctgrnm
				this.smTrplXpsCtgrChgYn = response.smTrplXpsCtgrChgYn || false
				if(this.smTrplXpsCtgrChgYn){
					this.smTrplXpsCtgrChgYn = "1"
				}else{
					this.smTrplXpsCtgrChgYn = "0"
				}
			})
		},
		reqSave(){
			...
			if(this.asetCtgrDsc === '1'){
				...
				data = {
					...
					// infOfrmnOrgC	: this.modalConfig.param.paramList.infOfrmnOrgC || "",	// 정보제공자기관코드
					// mydtCdIdVal		: this.modalConfig.param.paramList.mydtCdIdVal || "" ,	// 마이데이터카드식별자값
					// mydtAcno		: this.mydtAcno ,	// 마이데이터계좌번호
					// faceNo			: this.faceNo ,	// 권면번호
					// mydtScNo		: this.modalConfig.param.paramList.mydtScNo || "",	// 마이데이터회차번호
					// trDt			: this.trDt ,	// 거래일자
					// apvDt			: this.modalConfig.param.paramList.apvDt || "" ,	// 승인일자
					// accIdVal		: this.modalConfig.param.paramList.accIdVal	|| "" ,	// 계정식별값
					// cdApvno			: this.modalConfig.param.paramList.cdApvno || "",	// 카드승인번호
					// sqno			: this.modalConfig.param.paramList.sqno || null ,	// 일련번호
				...
				data = {
					...
					// this.$refs['minusMove'].checked?"1":"0"
					// dpzAcTrTpc 02출금 03 입금
				}
			}
			const config = {
