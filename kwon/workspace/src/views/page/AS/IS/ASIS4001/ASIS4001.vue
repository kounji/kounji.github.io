                        ...
                        <div class="text">
                            ...
                            <div class="toggle_money" :class="hideYn === true ? 'on' : ''">
                                <div class="sum">
                                    ...
                                    <span class="show">{{isrTotPymIsrfe | numberFilter}}원</span>
                                </div>
                                <button type="button" class="btns" @click="fn_setHidden('ISR', !hideYn)">
            ...
            <div class="com_inner">
                ...
                <div class="com_box_type01 toggle_list_box2 custom_list" v-if="insuCn > 0">
                    <div data-ui-toggle="box" class="toggle_box_area open">
                        <button type="button" class="view_btn" aria-expanded="true">
                            <div class="new_tit_area">
                                <div class="tit"><span>인보험</span> <span class="com_icon_num custom">{{insuCn}}<i class="blind">건</i></span></div>
                                <span class="total_price">
                                    <em class="num"><span class="blind">금액</span>{{mnTotPymIsrfe | numberFilter}}</em><em class="unit">원</em>
                                </span>
                            </div>
                            <em class="open">열기</em>
                            <em class="close">닫기</em>
                        </button>
                        <a href="javascript:void(0);" class="btn_sort" role="button" @click="fn_insuSortPop()"><span>{{insuSortNm}}</span></a>
                    </div>
                    <ul class="view_cont list_type_01">
                        <li v-for="(item, idx) in insuList" :key="'insu_'+idx">
                                ...
                                <dl>
                                    ...
                                    <dd>
                                        <span class="com_price">
                                            <em class="num"><span class="blind">금액</span>{{item.isrPymIsrfe | numberFilter}}</em><em class="unit">원</em><em class="int">{{item.isrPymFqNm}}</em>
                                        </span>
                                    </dd>
                                </dl>
                            ...
                            <!-- [v4.0] 청구하기 버튼 추가 -->
                            <div class="exe_btns" v-if="orgcSome(item.infOfrmnOrgC,item.isrCtrStsc)">
                ...
                <div class="com_box_type01 toggle_list_box2 custom_list" v-if="carInsuCn + tngInsuCn > 0">
                    <div data-ui-toggle="box" class="toggle_box_area open">
                        <button type="button" class="view_btn" aria-expanded="true">
                            <div class="new_tit_area">
                                <div class="tit"><span>물보험</span> <span class="com_icon_num custom">{{tngInsuCn + carInsuCn}}<i class="blind">건</i></span></div>
                                <span class="total_price">
                                    <em class="num"><span class="blind">금액</span>{{tngTotPymIsrfe | numberFilter}}</em><em class="unit">원</em>
                                </span>
                            </div>
                            <em class="open">열기</em>
                            <em class="close">닫기</em>
                            ...
                            <template v-if="item.type == 'car'">
                                <a href="javascript:void(0);" role="button" v-on:click.prevent="openDetailPop(item, 'main')" class="has_badge">
                                    <dl>
                                        ...
                                        <dd>
                                            <span class="com_price">
                                                <em class="num"><span class="blind">금액</span>{{item.tcarIsrIsrfe | numberFilter}}</em><em class="unit">원</em>
                                            </span>
                                        </dd>
                                    </dl>
                                    <span v-if="showStatus(item.isrCtrStsc)" :class="getStatusClass(item.isrCtrStsc)">{{item.isrCtrStsnm}}</span>
                ...
                <div class="com_box_type01 toggle_list_box2 custom_list" v-if="atisrInsuCn > 0">
                    <div data-ui-toggle="box" class="toggle_box_area open">
                        <button type="button" class="view_btn" aria-expanded="true">
                            <div class="new_tit_area">
                                <div class="tit"><span>인보험</span> <span class="com_icon_num custom">{{atisrInsuCn}}<i class="blind">건</i></span></div>
                                <span class="total_price">
                                    <em class="num"><span class="blind">금액</span>{{atMnTotPymIsrfe | numberFilter}}</em><em class="unit">원</em>
                                </span>
                            </div>
                            <em class="open">열기</em>
                            <em class="close">닫기</em>
                        </button>
                        <a href="javascript:void(0);" class="btn_sort" role="button" @click="fn_atisrInsuSortPop()"><span>{{atisrInsuSortNm}}</span></a>
                    </div>
                    <ul class="view_cont list_type_01">
                ...
                <div class="com_box_type01 toggle_list_box2 custom_list" v-if="(atisrTngInsuCn + atisrCarInsuCn) > 0">
                    <div data-ui-toggle="box" class="toggle_box_area open">
                        <button type="button" class="view_btn" aria-expanded="true">
                            <div class="new_tit_area">
                                <div class="tit"><span>물보험</span> <span class="com_icon_num custom">{{atisrTngInsuCn + atisrCarInsuCn}}<i class="blind">건</i></span></div>
                                <span class="total_price">
                                    <em class="num"><span class="blind">금액</span>{{atTngTotPymIsrfe | numberFilter}}</em><em class="unit">원</em>
                                </span>
                            </div>
                            <em class="open">열기</em>
                            <em class="close">닫기</em>
                        </button>
                        <a href="javascript:void(0);" class="btn_sort" role="button" @click="fn_atisrInsuTotalSortPop()"><span>{{atisrInsuTotalSortNm}}</span></a>
                    </div>
                    <ul class="view_cont list_type_01">
                        <li v-for="(item, idx) in atisrTngInsuList" :key="'atisrTngInsu_'+idx">
                            <a @click.prevent="openDetailPop(item, 'anti')" href="javascript:void(0);" role="button" class="has_badge">
                                <dl>
                                    ...
                                    <dd>
                                        <span class="com_price">
                                            <em class="num"><span class="blind">금액</span>{{item.isrPymIsrfe | numberFilter}}</em><em class="unit">원</em><em v-if="item.isrPymFqNm" class="int">{{item.isrPymFqNm}}</em>
                                        </span>
                                    </dd>
                                </dl>
                                <span v-if="showStatus(item.isrCtrStsc)" :class="getStatusClass(item.isrCtrStsc)">{{item.isrCtrStsnm}}</span>
...
export default {
    ...
    methods: {
        ...
        openDetailPop(insuInfo, type,isrCtrStsc) {
            let compName = "";
            console.log("insuInfo.isrKdDsc>>",insuInfo.isrKdDsc)
            //주계약자
            if(type == 'main') {
                if(this.insuIsrKdDsc.indexOf(insuInfo.isrKdDsc) > -1) {
                    let btnType = false
                    if((insuInfo.isrCtrStsc ==="02" || insuInfo.isrCtrStsc === "04") && insuInfo.infOfrmnOrgC === "B1AABF0000" && this.getUserInfo('chnl') === "386")btnType = true
                    insuInfo.btnType = btnType
        ...
        orgcSome(infOfrmnOrgC,isrCtrStsc){
            let btnType = false
            if((isrCtrStsc ==="02" || isrCtrStsc === "04") && infOfrmnOrgC === "B1AABF0000" && this.getUserInfo('chnl') === "386")btnType = true
            return btnType
