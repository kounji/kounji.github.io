<!--
/*************************************************************************
* @ 서비스경로 : 나의자산 > 자산 > 예금 > 외화상세
* @ 페이지설명 : 나의자산 > 자산 > 예금 > 외화상세
* @ 파일명     : src/views/page/AS/AC/ASAC1105/ASAC1105.vue
* @ 작성자     : CS515729
* @ 작성일     : 2021-06-17
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
			...
			<div class="com_inner">
				<div class="top_info_box_wrap">
					<div class="top_info_box type02 hasdetail new_top_box fixBG">
						...
						<div class="bill">
							<span class="num counter">{{respInfo.fcAcBacttAm | numberFilter}}</span><span class="text fs25">원</span>
						</div>
						<div class="new_global_price">
                            <template v-for="(item, idx) in dpAccDetailList">
                                <div class="price_wrap" :key="idx">
                                    <div class="won"><span class="num counter bold">{{item.acNowBac | numberFilter('0,0.00', {precision: 2})}}</span></div>
                                    <span class="badge">{{item.curc === 'KRW' ? '원' : item.curc+'('+item.curnm+')'}}</span>
                                </div>
                            </template>
						</div>
						<p class="text new_top_info_txt">{{respInfo.xcrtRgDt | dateFilter('YYYY년 MM월 DD일') }} 매도환율 기준입니다.</p>
						<div class="box_toggle_wrap">
							<div class="box_toggle">
								<div class="toggle-box">
									<div data-ui-toggle="box" class="no-stroke p0">									
										<div class="view-box" data-ui-toggle="view">
											<div class="info">
												<div class="title">이자율</div>
												<div class="num"><em>{{respInfo.aplItr | numberFilter('0,0.00', {precision: 2})}}%</em></div>
											</div>
											<div class="info">
												<div class="title">계좌개설일</div>
												<div class="text">{{respInfo.acOpDt | dateFilter('YYYY.MM.DD')}}</div>
											</div>
											<div class="info">
                    ...
                    <div class="scroll_wrap">
                        <div class="checkbox_box">
                            <span class="cmm_radio">
                                <input type="radio" name="chk" id="chk_all" value="" aria-hidden="true" v-model="srchFrag" v-on:click="fn_setSearchCond('')">
                                <label for="chk_all" role="radio" :aria-checked="srchFrag === '' ? 'true' : 'false'">전체</label>
                            </span>
                            <span class="cmm_radio">
                                <input type="radio" name="chk" id="chk_01" value="1" aria-hidden="true" v-model="srchFrag" v-on:click="fn_setSearchCond('1')">
                                <label for="chk_01" role="radio" :aria-checked="srchFrag === '1' ? 'true' : 'false'">입금</label>
                            </span>
                            <span class="cmm_radio">
                                <input type="radio" name="chk" id="chk_02" value="2" aria-hidden="true" v-model="srchFrag" v-on:click="fn_setSearchCond('2')">
                        ...
                        <div class="com_inner">
                            ...
                            <ul class="list_type_02"  :key="mIdx">
                                <li v-for="(item, dIdx) in monthObj.monList" :key="'day_'+dIdx">
                                    <dl>
                                        <dt>
                                            <div><em>{{item.dpzAcTrTpNm}}</em></div>
                                            <span>{{item.mydtTrDtm | dateFilter('DD일 HH:mm:ss')}}</span>
                                        </dt>
                                        <dd>
                                            <span class="com_price">
                                                <em class="num">{{item.dpzAcTram | numberFilter('0,0.00', {precision: 2})}}</em>
                                                <em class="unit">{{item.curc === 'KRW' ? '원' :  item.curnm}}</em>
                                            </span>
                                            <span class="com_price">
                                                <em class="num">{{item.acTrafBac | numberFilter('0,0.00', {precision: 2})}}</em>
                                                <em class="unit">{{item.curc === 'KRW' ? '원' : item.curnm}}</em>
                                            </span>
                                        </dd>
                                    </dl>
        ...
        methods: {
            ...
            getData() {
                ...
                apiService.call(config).then(response => {
                    ...
                    this.dpAccDetailList = this.respInfo.dpAccDetailList || []
            ...
            */
            fn_getDetailList(mod) {
				...
				apiService.call(config).then(response => {
                    ...
                    this.accTrList = this.fn_generateList(this.respDtlInfo.forgnAccTrList || [])
                })
            },
