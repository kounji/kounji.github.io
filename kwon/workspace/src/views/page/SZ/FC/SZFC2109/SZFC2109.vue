...
* @ 파일명     : src/views/page/SZ/FC/SZFC2109/SZFC2109.vue
* @ 작성자     : 
* @ 작성일     : 2021-06-15
************************** 수정이력 ****************************************
* 날짜                    작업자                변경내용
		...
		<div class="popup_content com_bg_type00">
			<div class="com_inner com_inputarea_type07">
				...
				<div class="com_input_type01">
					...
					<input type="text" data-popup="alert_popup" readonly id="txt01" class="input_cal_date input_left" name="" value="" v-model="trDt" ref="trDt" date-placeholder="" 
							title="날짜선택" @click="fn_popupCalendar($event)">
				</div>
				...
				<template v-if="InExRegister === 'I' || InExRegister === 'E'" >
					<div class="com_btnselectbox_type01 ani-active">
						<button type="button" class="com_btnselect_type01 com_iconstar" title="카테고리 선택" @click.prevent="fn_openSlidePop1($event)">
								<span>{{asetCtgrDsc1.length > 0 ? asetCtgrDsc1 : '카테고리를 선택해 주세요.'}}</span>
								<!-- <span>{{asetCtgrDsc1 === '1'?'수입 카테고리를 선택해 주세요.':'지출 카테고리를 선택해 주세요.'}}</span> -->
						</button>
					</div>
				</template>
				...
				<div class="com_btnselectbox_type01 ani-active">
					<button type="button" class="com_btnselect_type01 com_iconstar" title="품목 선택" @click.prevent="fn_openSlidePop3()">
						<span>{{naWrs.length > 0 ? naWrs : '품목을 선택해 주세요.'}}</span>
					</button>
				</div>
				...
				<template v-if="InExRegister === 'I' || InExRegister === 'E'" >
					<ul class="com_radio_chk ani-active">
						<li>
							<span title="현금" class="btn_radio">				 		  
				...
				<!--마이농가-수입 또는 지출일 경우(Start)-->
				<template v-if="InExRegister === 'I' || InExRegister === 'E' ">
					<div v-show="InpayMeth === '1' || ExpayMeth === '1'"  class="com_input_type01 ani-active com_won01 mb45">  <!--2021.11.16(화) 변경-->

						<input type="tel" id="com_input03" class="com_txtright_type01" :class="tram.length>0?'focusON ':''" name="" maxlength="17" required="" 
								@input="addComma('tram')" placeholder="금액을 입력하세요" title="금액을 입력하세요" v-model="tram" @keyup="moveNextTag($event)" ref="tram">
						<label for="com_input03">
							<em><span class="blind">필수입력</span></em><span class="txt_label_x">금액</span>
						</label>
						<div class="del_txt">
					...
					<div v-if="InpayMeth === '2' || ExpayMeth === '2'"  class="com_input_type01">

						<div class="com_btnselectbox_type01 ani-active">
							<button type="button" class="com_btnselect_type01 com_iconstar" @click.prevent="fn_openSlidePop2($event)"
									:title="InpayMeth === '2'?'수입항목 선택' : '지출항목 선택' ">

								<!-- <span>수입 항목 노출</span> -->
								<!-- <span>{{InpayMeth === '1'? '수입 항목 노출':'지출 항목 노출'}}</span> -->
					...
					<div class="com_input_type01 ani-active">	
						<input type="text" id="com_input05" class="com_txtright_type01" :class="trCntn.length>0?'focusON focusforce ':''" name="" :value="trCntn" ref="trCntn" required="" @input="fn_checkWord($event,50, 'trCntn')" 
								@keyup="moveNextTag($event)"  placeholder="거래처명 또는 입금자명을 입력해 주세요." title="거래처명 또는 입금자명을 입력해 주세요." >
						<label for="com_input01">
							<em><span class="blind">필수입력</span></em><span class="txt_label_x">거래처</span>
						</label>
						<!-- <div class="del_txt">
							<a class="com_btn_delete2" v-on:click.prevent="delStr('trCntn')" v-show="trCntn.length > 0"><span class="blind">삭제</span></a>
						</div> -->
					</div>

				</template>
				<!--마이농가-수입 또는 지출일 경우(End)-->

				<!--****************-->		
				<!--*****[하단]*****-->		
				<!--****************-->		
				<div class="com_textarea_type01 ani-active com_no_iconstar">
					<textarea name="" id="txt10" class="com_txtarea_type02" :value="memoCntn" ref="memoCntn" cols="10" rows="3" @input="fn_checkWord($event,100, 'memoCntn')"  required placeholder="메모를 입력하세요."></textarea> 
					<label for="txt00">
						<span class="txt_label_x">메모</span>
					</label>							
				</div>

            </div> <!--com_inner com_inputarea_type07(End)-->
		</div> <!--popup_content com_bg_type00(End)-->
        <!--메인(Body)-->	
        ...
        <div class="popup_footer fixed">
            <div class="btn_half_box">
				<template v-if="chkflag === '1' ">
					...
					<a href="javascript:void(0);" class="btn btn_mint" v-on:click.prevent="applyBtn('1')">저장</a>
                </template>
				<template v-else>
					<a href="javascript:void(0);" class="btn btn_grey" v-on:click.prevent="reset('2')">취소</a>
					<a href="javascript:void(0);" class="btn btn_mint" v-on:click.prevent="applyBtn('2')">저장</a>
                </template>
			</div>
        </div>
        <!--버튼(End)-->	
    ...
    methods: {
		...
		fn_openSlidePop1(e) {
            ...
            modalService.openSlidePagePopup(config).then(response => {
				...
				console.log("[SZFC2109]fn_openSlidePop1()/(변수)카테고리명=>>"+this.asetAmnCtgrId)  
