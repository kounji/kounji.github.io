...
* @ 서비스경로 : 공통
* @ 페이지설명 : 약관/동의 > 이용약관
* @ 파일명     : src/views/page/MR/AT/MRAT4001/MRAT4001.vue
* @ 작성자     : CS515731
* @ 작성일     : 2021-05-18
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
...
* 2025-05-14             CS541597                 청소년 모드 내 약관 노출 여부 설정(src/views/page/MR/AT/MRAT1001/MRAT1001.vue -> src/views/page/MR/AT/MRAT1004/MRAT1004.vue)
*************************************************************************/
-->
<template>
    <!-- full popup S -->
	<div class="full_popup renewal" id="full_popup_01"> 
		...
		<div class="popup_content com_no_bottom"><!-- 하단 버튼 없을때 com_no_bottom -->
			<div class="com_inner">
				<div class="terms_list_arrow" v-for="(item, index) in stltList" :key="index" v-show="setChildHide(item)">
					<strong class="com_pop_tit01">{{item.stltDsc == "2" ? "선택 동의" : "이용 약관"}}</strong>
					<ul>
                        <!-- 2025.05.14) 청소년모드 조합원약관 및 선택약관 미노출 -->
						<li v-for="(subItem, subIndex) in item.stltList" :key="subIndex" v-show="setChildHide(subItem)">
...
export default {
    ...
    methods: {
        ...
        /* 청소년모드 약관 미노출 */
        setChildHide(obj) {
            if(this.getUserMode !== 'C') return true
             ...
             * 2) 선택 약관
             */
            if(obj.stltTpc === 'PFM003' || obj.essYn === '0' || obj.stltDsc == '2') return false
            else return true
        },
        /* 약관 및 동의서 상세 팝업 열기 */
        openDetailPopup(obj) {
