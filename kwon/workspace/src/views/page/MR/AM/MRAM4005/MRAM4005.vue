...
* @ 페이지설명 : 연결자산관리 > 자산연결해제
* @ 파일명     : src/views/page/MR/AM/MRAM4005/MRAM4005.vue
* @ 작성자     : CS540685
* @ 작성일     : 2025-04-18
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
	...
	<div class="full_popup renewal mydata2023" id="full_popup_01"> 
		...
		<div class="popup_content"><!-- 하단 버튼 없을때 com_no_bottom -->
			<div class="com_inner">
				...
				<p class="desc">연결을 해제하면 고객님의 자산 정보를 더 이상 가져오지 않고, NH콕마이데이터에서 제공하는 정상적인 서비스 제공이 어려울 수 있어요.</p>
			</div>
			<div class="com_inner">
				<div class="com_box_type01 toggle_list_box2 custom_list inBtnType" v-for="(orgItem, orgIdx) in orgList" :key="'org_'+orgIdx">
					...
					<div class="view_cont">
						<div class="status_area">
							<p class="row">
								<strong class="tit">연결만료</strong>
								<span class="txt">{{orgItem.orgObj.tmsEdDt | dateFilter("YYYY.MM.DD")}}</span>
							</p>
							<p class="row">
								<strong class="tit">정기업데이트</strong>
								<span class="txt">{{orgItem.orgObj.fxtmTmsYn === '1' ? fn_getBscTmsFq(orgItem.orgObj.bscTmsFq) : "아니오"}}</span>
							</p>
							<p class="row">
								<strong class="tit">보유기간</strong>
								<span class="txt">서비스 이용 종료 또는 삭제요구시</span>
							</p>
						</div>
						<ul class="list_type_03">
							<li v-for="(orgWrsItem, orgWrsIdx) in orgItem.orgObj.wrsList" :key="'orgWrs_'+orgWrsIdx">
								<p class="bank_name">{{orgWrsItem.wrsnm}}</p>
								<p class="bank_num" v-for="(orgAccItem, orgAccIdx) in orgWrsItem.accList" :key="'orgAcc_'+orgAccIdx">
									{{orgAccItem.wrsCntn}}
								</p>
							</li>
						</ul>
