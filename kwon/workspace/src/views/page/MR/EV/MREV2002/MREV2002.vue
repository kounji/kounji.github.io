...
* @ 서비스경로 : 이벤트 확인 > 이벤트 응모
* @ 페이지설명 : 이벤트 응모
* @ 파일명     : src/views/page/MR/EV/MREV2002/MREV2002.vue
* @ 작성자     : CS533461
* @ 작성일     : 2023-10-19
************************** 수정이력 ****************************************
* 날짜                    작업자                변경내용
...
* 2024-02-21              CS530689              최초작성 (함께해용 이벤트)
*************************************************************************/
-->
<template>
    <!-- full popup S -->
	<div class="full_popup mydata2023 ev_bdragon" id="full_popup_01">
		...
		<div class="popup_content">
			...
			<!--상단비쥬얼-->
			<div><img src="@/assets_v40/images/event/ev_bd03.png" alt="NH콕마이데이터와 함께해용 이벤트 기간 2024년 3월 1일부터 3월 31일까지 스타벅스 기프트카드 만원권 500명 증정"></div>
			<!--이벤트내용-->
...
// import MREV2003 from '@/views/page/MR/EV/MREV2003/MREV2003'
...
export default {
    ...
    methods: {
        initComponent() {
			...
			this.evtStsc = this.params.evtStsc
        ...
        getData() {
			...
			apiService.call(config).then(res =>{
				...
				if(this.evtStsc == "0" || today > (res.edDt + res.edHr)){
		...
		fn_getEventData(){
			...
			apiService.call(config).then(response =>{
				...
				if(response.rsp_code == "0000"){
					...
					modalService.alert("오류가 발생 하였습니다.").then(() => {});
				}
