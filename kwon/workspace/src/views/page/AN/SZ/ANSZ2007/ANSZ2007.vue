<!--
/*************************************************************************
* @ 서비스경로 : 마이농가 > 설정 > 농기계 상각 등록 > 농기계 상각 등록
* @ 페이지설명 : 농기계 상각 등록
* @ 파일명     : src/views/page/AN/SZ/ANSZ2007/ANSZ2007.vue
* @ 작성자     : CS523299
* @ 작성일     : 2021-08-19 
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
...
* 2022-08-19             CS528061               고도화 및 파일명 변경(SZST2112 -> ANSZ2007)
*************************************************************************/
-->
<template>
    <!-- full popup S -->
	<div class="full_popup renewal" id="full_popup_01"> 
		...
		<div class="popup_content"><!-- 하단 버튼 없을때 com_no_bottom -->
			<div class="com_inner">
				<div class="custom_box">
					...
					<!-- 구입시기를 설정해 주세요 -->
					<div class="com_input_type01 com_word2 mt40 mb0" role="button" :aria-label="'필수입력, 구입시기'+buyDt+', 구입시기 선택'">
						<input type="text" data-popup="alert_popup" readonly="readonly" id="buyDt" class="input_cal_date com_txtright_type01 tar" name="" value="" 
					...
					<div class="com_input_type01 mt30 mb0">
						<input type="text" id="mftConm" name="mftConm" value="" maxlength="20" required="" placeholder="제조업체를 입력해 주세요" title="제조업체를 입력" v-model="mftConm">
						<label for="mftConm">
					...
					<div class="titbox_type01 style_up mt49">
						...
						<!--
							2022-07-27 : 마크업 삭제 => <span class="btn_area"></span>
						-->
					</div>
					...
					<ul class="bl_dot_list farm_list mt11">
						...
						<li>
							...
							(작목 비뮬을 0%로 지정 시 자동으로 나의농장이
							100%로 입력되고 상각계산 시 선택한 작목 비중은
							포함되지 않습니다.)
						</li>
					</ul>
					<!--// 작목 비율 설정 안내 글 -->
		...
		<div class="popup_footer fixed">
			<div class="com_btnhalfbox_type02">
				<a href="javascript:void(0);" role="button" class="btn btn_grey" @click.prevent="close()" aria-disabled="false">취소</a>
				<a href="javascript:void(0);" role="button" class="btn btn_mint" @click.prevent="fn_saveDprcInfo()" aria-disabled="false">저장</a>
				<!-- btn_off 제거시 활성화 -->
			</div>
		</div>
            ...
            fn_saveDprcInfo() {
                ...
                const config = {
                    ...
                    data: {
                        ...
                        asetTrryDsc : this.$refs['asetTrryDsc'].checked === true ? '03' : '01'  // 자산영역구분코드(마이농가에서 등록할 경우 : 01.마이농가자산, 03.마이농가자산+기타자산)
                    }
                }
            ...
            openSuccess() {
                const config = {
                    ...
                    params : {
                        ...
                        isAdd       : false,
                        isRlAsset   : true,
                        assetId     : "Agri",  // popId를 보내야하지만 SZST2108도 팝업이기 때문에 대신 assetId를 넘겨줌
                        stsDsc      : "I",
                    }
                }
