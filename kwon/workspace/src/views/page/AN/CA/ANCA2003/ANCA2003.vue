...
/*************************************************************************
* @ 서비스경로 : 제휴서비스 > 마이카 > 차계부
* @ 페이지설명 : 제휴서비스 > 마이카 > 차계부 지출내역
* @ 파일명     : src\views\page\AN\CA\ANCA2003\ANCA2003.vue
* @ 작성자     : CS533571
* @ 작성일     : 2023-08-18
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
        ...
        <div class="popup_content allianceSvc"><!-- 하단 버튼 없을때 com_no_bottom -->
            <div class="com_inner com_line_type01 com_top_type02 myCarReport">
                ...
                <div class="select_date">
                    <button class="cal_btn cal_prev" aria-label="한달 전 이동" @click.prevent="fn_moveMonth('P')" style="padding:0px 6px;"></button>
                    <a title="년월선택" class="year_month" href="javascript:void(0);" @click.prevent="openSlideMonthPop()"><span class="num">{{rzsBasYm | dateFilter('YYYY')}}</span>년 <span class="num">{{rzsBasYm | dateFilter('M')}}</span>월 </a>
                    <button class="cal_btn cal_next" :class="monNext ? '' : 'disabled'" aria-label="한달 후 이동" @click.prevent="fn_moveMonth('N')" style="padding:0px 6px;"></button>
                </div>

                <!-- 자동차 슬라이드 팝업 -->
                <a href="javascript:void(0);" data-popup="slide_popup_01" class="com_box_type01 myCarCho" @click.prevent="fn_openCarList(vhcnoVal)">
                    <div>
                        <div class="com_pop_tit01">{{carBrannm.concat(' ', carCrtpnm)}}<br><span>{{carGrdnm}}</span></div>
                ...
                <div class="chart_box">
                    ...
                    <div class="re_flex">
                        <div class="text_info gray">
                            월 평균 지출
                            <em class="ml6">{{rcnMt6AvgXpsAm | numberFilter}}</em>원
                        </div>
                    ...
                    <div class="tab_radius">
                        <ul role="">
                            <li v-for="(xpsTpInfo, index) in xpsTpList" :key="'key1_' + index">
                                <a href="javascript:void(0);" :data-tab-tooltip="'XP_DSC_' + xpsTpInfo.xpsTpDsc" role="button" 
                                    :class="inqXpsTpDsc == xpsTpInfo.xpsTpDsc ? 'active' : ''" @click.prevent="fn_initCall(xpsTpInfo.xpsTpDsc)" 
                                    :ref="'XP_DSC_' + xpsTpInfo.xpsTpDsc">{{xpsTpInfo.xpsTpDsNm}}</a>
                            </li>
                        </ul>
                    </div>
                </template>
                        ...
                        <ul>
                            <template v-for="(basYmXpsBrkInfo, index2) in basYmXpsBrkList" :data="basYmXpsBrkInfo">
                                <li :key="index2" v-if="basYmXpsBrkInfo.trDt == mstBasYmXpsBrkInfo.trDt">
                                    <i :class="basYmXpsBrkInfo.carbkCtgrId"></i>
            ...
            <div class="com_btn_area fixed">
                <a href="javascript:void(0);" role="button" class="com_btn_round" @click="assetAdd('1')">
    ...
    methods: {
        ...
        getData() {
            ...
            apiService.call(config).then(response => {
                ...
                this.rcnMt6XpsList.forEach(d => {
                    chartData.push({ "Month" : dateFormat(d.rzsYm, 'M월'), "지출 금액" : d.rzsXpsAm })
                })
        ...
        */
		...
		var layoutStr = '<rMateChart backgroundColor="#FFFFFF" borderStyle="none">'
                ...
                */
                +'<Column2DChart gutterLeft="0" showDataTips="false" selectionMode="single" columnWidthRatio="0.48" showHorizontalAxisDataTip="true" showVerticalAxisDataTip="true" axisDataTipJsFunction="axisDataTipFunc" displayCompleteCallFunction="rMateFunction.removeTabindex" styleName="valueLabelStyle">'
                    /*천단위 , 표시*/
