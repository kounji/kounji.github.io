...
* @ 서비스경로 : 연금절세 > 노후준비
* @ 페이지설명 : 메인
* @ 파일명     : src\views\page\PD\RT\PDRT4001\PDRT4001.vue
* @ 작성자     : CS528069
* @ 작성일     : 2022-09-22
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
...
* 2025-01-06              CS541015              고도화 파일명, UI변경 및 고도화 개발(PDRT2001.vue -> PDRT4001.vue)
*************************************************************************/
-->
<template>
		...
		<div id="content" class="renewal old_age">
			...
			<div class="com_inner" v-if="prtoDsc =='1'">
				...
				<div class="com_box_type01 bottomBtnType">
						...
						<div class="custom_box_chart old_age_chart">
							...
							<div class="compare_chart_wrap" v-if="sMmLfecs > mmPnsTts">
								...
								<div class="chart_info">
									<p>
										안정된 노후 생활을 위해<br>
										<strong><span class="pointColor red2">월 {{fn_hanValue(String(sMmLfecs-mmPnsTts))}}</span> 더 필요해요.</strong>
									</p>
								</div>	
							</div>
					...
					<button class="bottomBtn" @click.prevent="showMore">더보기</button>
				</div>
			</div>
			...
			<hr class="hr03">
			<div class="com_inner">
				<strong class="titH3">노후준비를 위해 모은 돈 <span class="num">{{allPnsTts| numberFilter}}</span>원</strong>
				...
				<div class="com_box_type01 toggle_list_box2 custom_list" v-if="sNtpsTotAm > 0" >
					...
					<ul class="view_cont list_toggle">
						<li>
							...
							<a href="javascript:void(0);" class="link_arrow txt_r" @click.prevent="fn_PDSC4001()" >연금가입내역</a>
						</li>
					</ul>
				</div>
				<!-- //국민연금 -->
						...
						<li v-for="(item, idx) in pnsvInsuList" :key="`0_${idx}`">
							<a href="javascript:void(0);" role="button" @click.prevent="fn_openDetailPop('PSIS', item)">
								<dl>
									...
									<dd>
										<span class="prod_num">월 납입액 {{item.isrPymIsrfe | numberFilter}}원</span>
									</dd>
									<dd>
										<span class="com_price">
			...
			<div class="new_add_register"><a href="javascript:void(0);" role="button" @click.prevent="fn_movePopup('COAR4002')"><span>연금이 더 있나요?</span></a></div>
