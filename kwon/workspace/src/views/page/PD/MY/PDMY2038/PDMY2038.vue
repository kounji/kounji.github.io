<!--
/*************************************************************************
* @ 서비스경로 : 금융과생활 > 나의목표 > 목표등록 > 금융목표
* @ 페이지설명 : 금융과생활 > 나의목표 > 목표등록 > 금융목표 > (돈모으기)Step 1. 정보 입력 (타이틀&금액) 돈모으기 상세(등록/상세 공용)
* @ 파일명     : src/views/page/PD/MY/PDMY2038/PDMY2038.vue
* @ 작성자     : CS516029
* @ 작성일     : 2021-06-03
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
...
* 2022-06-29              CS528053                 고도화 파일명, UI 변경 및 고도화 개발(PDMY1106.vue -> PDMY1003.vue)
* 2023-08-04              CS533453                 고도화 파일명 UI 변경(PDMY1003.vue -> PDMY2038.vue)
*************************************************************************/
-->
		...
		<div class="popup_content">
            <div class="com_inner com_line_type01 com_top_type02 pt1">
				<div class="titbox_type01 style_up mt0">
					<strong class="com_pop_tit01">돈 모으기 목표를 설정하세요.</strong>
				</div>
			</div>
			...
			<div class="com_inner">
				...
				<div class="price_wrap">
					<div class="com_input_type01 com_word2">
						<input type="tel" id="com_input02" class="input_right" name="" value="" v-model="fncObtAm" ref="fncObtAm" required placeholder="얼마를 모을까요" title="목표평가금액 입력" maxlength="8" @keyup="addComma($event)" @keyup.enter="moveNextTag($event)" @focus="fn_focusOnOff('fncObtAm')">
				...
				<div class="com_input_type01" role="button" @click="fn_popupCalendar($event)" :aria-label="'필수입력,' + getDateFormat(obtDt, '', '목표기간') + '목표기간 선택'">
					<input type="text" id="com_input03" class="input_cal_date input_right" name="" value="" v-model="obtDt" data-popup="alert_popup" readonly required placeholder="언제까지 모을까요(최소1개월~10년사이)" title="언제까지 모을까요" aria-hidden="true" @focus="fn_focusOnOff('obtDt')">
                ...
                <template v-if="subList.length > 0">
                    <div class="com_txt">
                        ...
                        <p>매 월 <strong class="com_point_blue">{{dayAm | numberFilter}}원</strong>씩 모으면 가능해요!</p>
                    </div>
                    <div>
                ...
                <template v-else>
                    <div class="connect_asset custom">
                        ...
                        <p>
                            선택된 예/적금계좌 및 요구불계좌의 잔액이<br/>돈모으기 목표 달성의 기준이 됩니다.
                        </p>
                        <div class="com_btn_area" v-if="prgStsCd == '1'"><a href="javascript:void(0);" class="com_btn_round" @click="openSlidePopup()"><span class="btn_plus black">연결계좌 선택</span></a></div>
                    </div>
                </template>
				<!-- //계좌선택전 -->
			</div>
