<!--
/*************************************************************************
* @ 서비스경로 : 전체메뉴 > 도전목표챌린지 > 버킷리스트 > 여가생활 목표
* @ 페이지설명 : 전체메뉴 > 도전목표챌린지 > 버킷리스트 > 여가생활 목표 > 여가생활 목표 저장
* @ 파일명     : src\views\page\PD\MY\PDMY4006\PDMY4006.vue
* @ 작성자     : CS533453
* @ 작성일     : 2023-07-17
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
...
* 2025-01-13              CS541015              고도화 파일명, UI변경 및 고도화개발(PDMY2006.vue -> PDMY4006.vue)
*************************************************************************/
-->
...
<template>
	<div class="full_popup mydata2023" id="full_popup_01"> 
		...
		<div class="popup_content com_bg_type00">
			<div class="com_inner">
				<strong class="titH1">힐링 라이프, 나의 여가생활!</strong>

				<div class="com_inputarea_type01">					
					<div class="com_input_type01">
						<input type="text" ref="obtnm" id ="obtnm" v-model="obtnm" required placeholder="목표 제목 입력" title="목표 제목" @input="inputCtr($event)">
					...
					<!-- 여가생활 카테고리 선택(인풋창 아님) 클릭시 팝업창 호출 -->
					<div class="com_btnselectbox_type01">
						<button type="button" class="com_btnselect_type01 search_icon" title="여가생활 선택" @click.prevent="openComCodePop(1)">
							<template v-if="lsrLfeCstRkonDscNm1 == ''">
								여가생활 선택
							</template>
							<template v-else>
								<span>{{lsrLfeCstRkonDscNm1}}</span>
							</template>
						</button>
					</div>
					<!-- //여가생활 카테고리 선택(인풋창 아님) 클릭시 팝업창 호출 -->
					...
					<!-- 여가생활 항목 카테고리 선택 -->
					<div class="com_btnselectbox_type01">
						<button type="button" class="com_btnselect_type01" title="여가생활 항목 선택" @click.prevent="openComCodePop(2)">
							<template v-if="lsrLfeCstRkonDscNm2 == ''">
								항목 선택
							</template>
							<template v-else>
								<span>{{lsrLfeCstRkonDscNm2}}</span>
							</template>
						</button>
					</div>
					<!-- //여가생활 항목 카테고리 선택 -->
					...
					<!-- 유형 선택 -->
					<div class="com_btnselectbox_type01">
						<button type="button" class="com_btnselect_type01" title="유형 선택" @click.prevent="openComCodePop(3)">
							<template v-if="lsrLfeCstRkonDscNm3 == ''">
								유형 선택
							</template>
							<template v-else>
								<span>{{lsrLfeCstRkonDscNm3}}</span>
							</template>
						</button>
					</div>
					<!-- //유형 선택 -->
					...
					<!-- 비용 -->
					<div class="cost_wrap">
						<strong class="titH4">{{lsrLfeCstRkonDscNm2}} 비용 (1년 기준)</strong>
						<div class="list_gray_box">
							<dl v-for="(codeInfo4, index) in codeListDepth4" :key="'key1_'+index">
								<dt>{{codeInfo4.comnCExpl}}</dt>
								<dd><span class="num">{{codeInfo4.refCntn3}}</span>원</dd>
							</dl>
							<!-- <dl>
								<dt>시설 이용료</dt>
		...
		<div class="popup_footer fixed">
			<div class="btn_full_box">
				<a @click.prevent="insertExpensePerpose()" class="btn btn_mint" href="javascript:void(0);">
					<template v-if="insYn === 'C'">저장</template><!--[v4.0] 문구수정 -->
...
import PDMY2035 from '@/views/page/PD/MY/PDMY2035/PDMY2035' // 여가생활 카테고리 조회 팝업
...
export default {
    ...
    methods: {
		init() {
			this.lsrLfeCstRkonDsc1		= ""	// 여가생활비용산정구분코드1
			this.lsrLfeCstRkonDscNm1	= ""	// 여가생활비용산정구분코드명1
			this.lsrLfeCstRkonDsc2		= ""	// 여가생활비용산정구분코드2
			this.lsrLfeCstRkonDscNm2	= ""	// 여가생활비용산정구분코드명2
			this.lsrLfeCstRkonDsc3		= ""	// 여가생활비용산정구분코드3
			this.lsrLfeCstRkonDscNm3	= ""	// 여가생활비용산정구분코드명3
			this.chkObtDt				= ""
		},
		initComponent(params) {
			...
			if (params !== undefined && params !== null 
				&& params.fncObtDsc !==undefined && params.fncObtDsc !== null && params.fncObtDsc === '06' 
		...
		// 여가생활카테고리조회 팝업
		openCategoryPop() {
			// let compName = PDMY2035;

            let param = {
				comnCId : "LSR_LFE_CST_RKON_DSC1"
			}
            const config = {
                component: PDMY2035,
                params : param
            ...
            modalService.openPopup(config).then(response => {
                if(response != null) {
					this.lsrLfeCstRkonDsc1		= response.comnCVal
					this.lsrLfeCstRkonDscNm1	= response.comnCExpl
		...
		openComCodePop(depth) {
			...
			if(depth == 1) {
				...
				if(this.lsrLfeCstRkonDsc1 == '') {
					modalService.alert("여가생활을 선택해주세요").then(() => {});
					return
				}
				comnCId		= "LSR_LFE_CST_RKON_DSC2"
				refCntn1	= this.lsrLfeCstRkonDsc1
				comnCExpl	= this.lsrLfeCstRkonDscNm1 + " 선택"
			} else if(depth == 3) {
				if(this.lsrLfeCstRkonDsc2 == '') {
					modalService.alert("항목을 선택해주세요").then(() => {});
					return
				}
				comnCId		= "LSR_LFE_CST_RKON_DSC3"
				refCntn1	= this.lsrLfeCstRkonDsc2
				comnCExpl	= "유형 선택"
			} else if(depth == 4) {
				refCntn1 = this.lsrLfeCstRkonDsc3
				this.getDetailCodeList('LSR_LFE_CST_RKON_DSC4', refCntn1, null)
				return
			} 
				...
				if(response != null) {
					if(response.comnCId != null && response.comnCId !== "") {
						...
						if(depth == 1) {
							this.lsrLfeCstRkonDsc1 = response.comnCVal
							this.lsrLfeCstRkonDscNm1 = response.comnCExpl
							this.lsrLfeCstRkonDsc2 = ""
							this.lsrLfeCstRkonDscNm2 = ""
							this.lsrLfeCstRkonDsc3 = ""
							this.lsrLfeCstRkonDscNm3 = ""
							this.codeListDepth4 = []
						} else if(depth == 2) {
							this.lsrLfeCstRkonDsc2 = response.comnCVal
							this.lsrLfeCstRkonDscNm2 = response.comnCExpl
							this.lsrLfeCstRkonDsc3 = ""
							this.lsrLfeCstRkonDscNm3 = ""
							this.codeListDepth4 = []
						} else if(depth == 3) {
							this.lsrLfeCstRkonDsc3 = response.comnCVal
							this.lsrLfeCstRkonDscNm3 = response.comnCExpl
							this.codeListDepth4 = []
						}
						this.$nextTick(()=>{
		...
		getDetailCodeList(comnCId, refCntn1, fncObtAm) {
			...
			apiService.call(config).then(response => {
				this.codeListDepth4 = response.comnCList || []
				if(fncObtAm == null) {
		...
		getData() {
			...
			apiService.call(config).then(response => {
				...
				this.getDetailCodeList("LSR_LFE_CST_RKON_DSC4", this.lsrLfeCstRkonDsc3, this.fncObtAm)
			});
		},
        // 글자수 체크
		...
		insertExpensePerpose() {
			...
			if(this.lsrLfeCstRkonDsc1.length == 0 || this.lsrLfeCstRkonDsc1 == '') {
				modalService.alert("여가생활은 필수입력항목입니다.").then(() => {});
				return;
			}
			if(this.lsrLfeCstRkonDsc2.length == 0 || this.lsrLfeCstRkonDsc2 == '') {
				modalService.alert("항목은 필수입력항목입니다.").then(() => {});
				return;
			}
			if(this.lsrLfeCstRkonDsc3.length == 0 || this.lsrLfeCstRkonDsc3 == '') {
				modalService.alert("유형은 필수입력항목입니다.").then(() => {});
				return;
			}
		...
		fn_save() {
			...
			const config = {
				...
				data: {"trDsc":this.insYn
						...
						,"lsrLfeCstRkonDsc1" : this.lsrLfeCstRkonDsc1
						,"lsrLfeCstRkonDsc2" : this.lsrLfeCstRkonDsc2
						,"lsrLfeCstRkonDsc3" : this.lsrLfeCstRkonDsc3
		...
		fn_bannerPopYn() {
			...
			for(let i=0; i<modalLength; i++) {
				if(modalInfo[i].component.name.indexOf("PDYT4105") !== -1 ||
					modalInfo[i].component.name.indexOf("PDRT2016") !== -1 ||
