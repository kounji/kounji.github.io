...
* @ 파일명     : src\views\page\PD\YT\PDYT1103\PDYT1103.vue
* @ 작성자     : CS515897
* @ 작성일     : 2021-08-25
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
...
<template>
	<div class="full_popup">
		...
		<!-- content S -->
		<!-- 연결된 자산이 있는경우 -->
		<div class="com_sub_type01 com_bg_type00 popup_content" v-if="!isAllNotExist">
			<div class="com_inner com_line_type01">
				<div class="com_cont_tit01" v-if="isNotTotAmt">
					<!-- 공제금액이 모두없는경우 -->
					<div class="tit"><strong>{{cusNm}}님</strong>, 추가로 연결할<br>자산이 있지 않나요?</div>
					<a href="javascript:void(0);" class="com_btnround" @click.prevent="fn_openCOAR2001()"><span class="btn_plus">자산등록</span></a>
					<!-- //공제금액이 모두없는경우 -->
				</div>
				<div class="com_cont_tit01" v-else>
					<!-- 공제금액이 있는경우 -->
					<div class="tit"><strong>{{cusNm}}님</strong>, 연말정산으로<br><strong>공제 받을 수 있는 금액</strong>이 있습니다.</div>
					<div class="txt">아래 금액은 연결된 자산 기준으로 계산된 예상<br>금액으로 <span class="com_point_blue">실제 공제 금액과는 다를 수 있습니다.</span></div>
					<!-- //공제금액이 있는경우 -->
				</div>
			</div>
			<div class="com_inner com_boxarea_type01">
				...
				<template v-if="isCdConnect">
					<!-- 공제금액이 있는경우 -->
					<div class="com_box_type01 com_box_list04" v-if="cd_ovrYn === '1'">
						<a href="javascript:void(0);" @click.prevent="fn_openPDYTDtl('card')">
							<div class="new_tit_area">
								<div><span class="icon_blue">카드소득공제</span></div>
								<div class="txt">최대 <span>{{cd_maxMadAm | numberFilter}}원</span> 까지 소득공제 받을 수 있습니다.</div>
							</div>
							<!-- 설명부분 -->
							<div class="info_list"><strong>체크카드 사용</strong>이 소득공제에 유리합니다.</div>
							<!-- //설명부분 -->
						</a>
					</div>
					<!-- //공제금액이 있는경우 -->
					...
					<!-- 공제금액이 모두없는경우 -->
					<div class="com_box_type01 com_box_list04" v-else>
						<a href="javascript:void(0);" @click.prevent="fn_openPDYTDtl('card')">
							<div class="new_tit_area">
								<div><span class="icon_blue">카드소득공제</span></div>
								<div class="txt">소득공제 금액이 없습니다.</div>
							</div>
							<!-- 설명부분 -->
							<div class="info_list">카드사용금액 <span v-html="fn_numberToKorea(cd_minCdUsAm)"></span>이 넘어야 소득공제 받을 수 있습니다.</div>
							<!-- //설명부분 -->
						</a>
					</div>
					<!-- //공제금액이 모두없는경우 -->
				...
				<template v-if="isHseConnect">
					<!-- 공제금액이 있는경우 -->
					<div class="com_box_type01 com_box_list04" v-if="hse_madAm > 0">
						<a href="javascript:void(0);" @click.prevent="fn_openPDYTDtl('subscription')">
							<div class="new_tit_area">
								<div><span class="icon_blue">청약소득공제</span></div>
								<div class="txt">납입한 금액 중 <span>{{hse_madAm | numberFilter}}원</span>을 소득공제 받을 수 있습니다.</div>
							</div>
							<!-- 설명부분 -->
					...
					<div class="com_box_type01 com_box_list04" v-else>
						<a href="javascript:void(0);" @click.prevent="fn_openPDYTDtl('subscription')">
							...
							<!-- 설명부분 -->
							<div class="info_list"><span>7천만원</span> 이하의 연소득 근로소득자로 납입 금액이 있어야 합니다.</div>
							<!-- //설명부분 -->
						</a>
					</div>
					<!-- //공제금액이 모두없는경우 -->
				...
				<template v-if="isIrpConnect">
					<!-- 공제금액이 있는경우 -->
					<div class="com_box_type01 com_box_list04" v-if="irp_madAm > 0">
						<a href="javascript:void(0);" @click.prevent="fn_openPDYTDtl('irp')">
							<div class="new_tit_area">
								<div><span class="icon_mint">연금세액공제</span></div>
								<div class="txt">납입한 금액 중 <span>{{irp_madAm | numberFilter}}원</span>을 소득공제 받을 수 있습니다.</div>
							</div>
							<!-- 설명부분 -->
							<div class="info_list">노후 준비 자금과 공제금액을 확인하세요.</div>
							<!-- //설명부분 -->
						</a>
					</div>
					<!-- //공제금액이 있는경우 -->
					...
					<div class="com_box_type01 com_box_list04" v-else>
						<a href="javascript:void(0);" @click.prevent="fn_openPDYTDtl('irp')">
							<div class="new_tit_area">
								<div><span class="icon_mint">연금세액공제</span></div>
								<div class="txt">세액공제 금액이 없습니다.</div>
							</div>
							<!-- 설명부분 -->
				...
				<template v-if="isCdConnect || isHseConnect || isIrpConnect || isInsuConnect">
					<!-- 공제금액이 있는경우 -->
					<div class="com_box_type01 com_box_list04" v-if="mdc_madAm > 0">
						<a href="javascript:void(0);" @click.prevent="fn_openPDYTDtl('medical')">
							<div class="new_tit_area">
								<div><span class="icon_mint">의료비세액공제</span></div>
								<div class="txt">의료비 사용금액 중 <span>{{mdc_madAm | numberFilter}}원</span>을 세액공제 받을 수 있습니다.</div>
							</div>
						</a>
					</div>
					<!-- //공제금액이 있는경우 -->
					...
					<div class="com_box_type01 com_box_list04" v-else>
						<a href="javascript:void(0);" @click.prevent="fn_openPDYTDtl('medical')">
							...
							<!-- 설명부분 -->
							<div class="info_list">의료비 사용금액이 <span v-html="fn_numberToKorea(mdc_midMdxpUsAm)"></span>이 넘어야 세액공제 받을 수 있습니다.</div>
							<!-- //설명부분 -->
						</a>
					</div>
					<!-- //공제금액이 모두없는경우 -->
				...
				<template v-if="isInsuConnect">
					<!-- 공제금액이 있는경우 -->
					<div class="com_box_type01 com_box_list04" v-if="insu_madAm > 0">
						<a href="javascript:void(0);" @click.prevent="fn_openPDYTDtl('insu')">
							<div class="new_tit_area">
								<div><span class="icon_mint">보험료세액공제</span></div>
								<div class="txt">납입한 보험료 중 <span>{{insu_madAm | numberFilter}}원</span>을 세액공제 받을 수 있습니다.</div>
							</div>
						</a>
					</div>
					<!-- //공제금액이 있는경우 -->
					...
					<div class="com_box_type01 com_box_list04" v-else>
						<a href="javascript:void(0);" @click.prevent="fn_openPDYTDtl('insu')">
							...
							<!-- 설명부분 -->
							<div class="info_list">일반보장성보험료 납입 내역이 없습니다.</div>
							<!-- //설명부분 -->
						</a>
					</div>
					<!-- //공제금액이 모두없는경우 -->
...
import PDYT1104 from '@/views/page/PD/YT/PDYT1104/PDYT1104'
import PDYT1105 from '@/views/page/PD/YT/PDYT1105/PDYT1105'
import PDYT1106 from '@/views/page/PD/YT/PDYT1106/PDYT1106'
import PDYT1107 from '@/views/page/PD/YT/PDYT1107/PDYT1107'
import PDYT1108 from '@/views/page/PD/YT/PDYT1108/PDYT1108'
