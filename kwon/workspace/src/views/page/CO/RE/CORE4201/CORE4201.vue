...
/*************************************************************************
* @ 서비스경로 : 자산등록 > 부동산
* @ 페이지설명 : 자산등록 > 부동산 > 부동산 종류 선택
* @ 파일명     : src/views/page/CO/RE/CORE4201/CORE4201.vue
* @ 작성자     : CS541013
* @ 작성일     : 2025-01-13
************************** 수정이력 ****************************************
* 날짜                    작업자                변경내용
...
<template>
    <div class="full_popup renewal" id="full_popup_01">
            ...
            <div class="com_inner">
                ...
                <template :class="isApt ? 'on' : ''" v-if="isApt">
                    ...
                    <template v-if="isAptSrch">
                        ...
                        <div class="com_btnselectbox_type01">
                            <button type="button" title="주거형태 선택" class="com_btnselect_type01" @click.prevent="openRlestAreaPop()" ref="rlestDsc">
                ...
                <template :class="!isApt ? 'on' : ''" v-else-if="!isApt">
                    <div class="tab_wrap02">
                        ...
                        <!-- 주택(5)/오피스텔(2)/상가(7)/기타(9) 선택 시 -->
                        <div class="com_btnselectbox_type01" v-if="((rlestTngDsc == '5' || rlestTngDsc == '2' || rlestTngDsc == '7' || rlestTngDsc == '9') && (rlestNm != '' || rlestNmShow))">
                        ...
                        <template v-if="((rlestTngDsc == '5' || rlestTngDsc == '2'|| rlestTngDsc == '6' || rlestTngDsc == '7' || rlestTngDsc == '9') && rlestRsdFormDsc != '')">
                            <div class="switch_list_wrap">
                                <!-- 자가이거나 부동산 물건구분코드가 토지/농지(6), 상가(7), 기타(9)인 경우 show -->
                                <div class="custom_list com_box_type01" v-if="rlestRsdFormDsc == '1'">
		...
		<div class="popup_footer fixed">
			...
			<div class="btn_full_box" v-if="isApt & !isAptSrch">
				<a href="javascript:void(0);" role="button" class="btn btn_mint" :class="btnOnOff" @click.prevent="openAddrSrch" :aria-disabled="btnOnOff == 'btn_off' ? true : false">다음</a>
			</div>

            <!-- 아파트 선택, 아파트 검색하면 btnon  -->
            <div class="btn_full_box" v-else-if="isApt & isAptSrch">
            ...
            <!-- 주택, 오피스텔, 토지/농가, 상가, 기타 선택, 모든 기입란 입력하면 btnon  -->
        ...
        */
        ...
        openAddrSrch(e) {
            ...
            modalService.openPopup(config).then(response => {
                this.aptNm          = response.aptNm            // 아파트명(부동산명)
                this.aptHscxIdvdc   = response.aptHscxIdvdc     // 아파트단지개별코드
                this.isAptSrch      = true
                ...
                // v4.0 추가 start
                this.rlestRsdFormDsc    = response.rlestRsdFormDsc	    // 주거형태 (자가:1, 전세:2, 월세:3)
                this.rlestRsdFormDscNm  = response.rlestRsdFormDscNm	// 주거형태명
                // v4.0 추가 end
        ...
        openRlestAreaPop(e) {
            ...
            const config0 = {
                ...
                data : {
                    ...
                    rlestRsdFormDsc : this.rlestRsdFormDsc,
                    rlestRsdFormDscNm : this.rlestRsdFormDscNm
                }
            }
            apiService.call(config0).then(response => {
                ...
                const config1 = {
                    params : {
                        ...
                        selectNm    : this.rlestRsdFormDscNm,   // 주거형태명
                        list        : listData,
                        isApt       : this.isApt
                    },
                    renderer : {
