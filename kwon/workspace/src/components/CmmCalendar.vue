...
<template>
  <!-- <div> -->
      <table class="cal_tb">
        ...
        <thead>
          <tr>
            <th v-for="day in days" :key="day" :class="getDayOfClass(day)">{{ day }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(date, idx) in dates" :key="idx" :id="'date_' + idx">
            <td v-for="(day, secondIdx) in date" :key="secondIdx" :id="'day_' + idx + '_'+ secondIdx" :class="[day.dayOfWeekClass , (day.ymd === today) ? 'today':'']">
                  <template v-if="(day.dayOfWeekClass !== 'pre')">
                    ...
                    :aria-label="`${bizType !== 'FR' ? (day.dates + '일, ') + (day.rzs && day.rzs.xpsAm && day.rzs.xpsAm != 0 ? '지출' + day.rzs.xpsAm + '원, ' : '') + (day.rzs && day.rzs.revAm && day.rzs.revAm != 0 ? '수입' + day.rzs.revAm + '원' : '') : ''}`">
                      <span 
                        :class="`date ${getDateOfClass(day.dayOfWeekClass)}`">
                        {{ day.dates }}
                      </span>
    ...
    watch:{
      pYYYYMM: function(newVal, oldVal)
      {
        ...
        if (newVal != oldVal)
        {
          ...
          //달력 그리고 난 후 1일 focus 처리
          this.$nextTick(() => {
            this.sDate = newVal + '01'
          })
        }
      },
