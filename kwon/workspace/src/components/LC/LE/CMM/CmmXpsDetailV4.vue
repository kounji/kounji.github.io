...
import LCLE4119 from '@/views/page/LC/LE/LCLE4119/LCLE4119'
...
import LCLE4016 from '@/views/page/LC/LE/LCLE4016/LCLE4016'
...
export default {
    ...
    data: () => {
        return {
            ...
            isXpsEst        : false,    // 지출설정 여부
            xpsEstAm        : 0,        // 지출설정금액
            xpsEstDsc       : "01",     // 지출설정주기 01.매월초기화, 02매월자동반영
            xpsDifAm        : 0,        // 지출차이금액
            xpsDifLabel     : "",       // 지출차이라벨
    ...
    methods: {
        ...
        setCurrentMethod(param) {
            console.log("setCurrentMethod() param===================", param)
            switch(this.type) {
                case TYPE_CARD:
        ...
        // 요청 파라미터
        getRequestData() {
            console.log("getRequestData() start===================")
            let data = {
            ...
            console.log("getRequestData() data===================", data)
        ...
        assignData(obj) {
            ...
            this.xpsDifAm = this.xpsEstAm - this.sam; // 지출목표차이금액
            
            // 지출목표차이라벨
            if (this.xpsDifAm < 0) {
                this.xpsDifLabel  = "초과 되었어요."
                this.xpsDifAm = this.xpsDifAm * -1
            } else {
        ...
        fn_initSrch(){
            ...
            // 조건검색 파라미터
            this.isSrch			= this.isSrchProp    // 조건검색 여부
            this.isXpsEst       = this.isXpsEstProp  // 지출목표설정 여부
            this.prcMchtnmCnd	= ""            // 가맹점명 검색조건
            this.cdTyCnd		= ""            // 카드종류 검색조건
            this.cdTyCndLabel   = ""            // 카드종류 검색조건 라벨
        ...
        // 결제취소 여부
        isCanceled(item) {
            console.log("isCanceled() item===================", item)
            if(this.type == TYPE_CARD || (this.type == TYPE_PAY && this.subtype == SUBTYPE_PPAYCARD)) {
                if(item.stlStsc == '02') {
                    return true
                }
        ...
        // 페이머니 상세 팝업
        openPayMnyDetail() {
            console.log("openPayMnyDetail() start===================")
        ...
        // 선불카드 상세 팝업
        openPpayCdDetail() {
            console.log("openPpayCdDetail() start===================")
        ...
        // 선택 슬라이드팝업.
        openSelectSlidePop() {
            console.log("openSelectSlidePop() start===================")
            let calledId = ''
            let selectVal = ''
            if(this.type == TYPE_PAY && this.subtype == SUBTYPE_PAYMONEY) {
                selectVal = this.faceOnm + this.accIdVal2
                calledId = 'LCLE4003'
            }else{
        ...
        openInfoSlidePop(obj) {
            console.log("openInfoSlidePop() start===================")
            const config = {
                params: obj,
                renderer : {
        ...
        // 지출목표설정 슬라이드 팝업
        openXpsExtSlidePop() {
            console.log("openXpsExtSlidePop() start===================")
            const config = {
                component : LCLE4016,
                params : {
                    inqYm           : this.inqYm,
            ...
            console.log("openXpsExtSlidePop() response===================", response)
                if(response.isXpsEst !== undefined) {
                    ...
                    if (this.xpsEstAm != tmpXpsEstAm || this.xpsEstDsc  != response.xpsEstDsc) {
                        this.xpsEstAm   = tmpXpsEstAm
                        this.xpsEstDsc  = response.xpsEstDsc

                        this.setStlMnsXpsObtData()
                        this.callJQueryFncExcute()
                    }                    
                }
                this.xpsEstAm = 0
        ...
        setStlMnsXpsObtData() {
            console.log("setStlMnsXpsObtData() start===================")
            let estMydtCdIdVal = ''
            let estAccIdVal    = ''

            switch(this.type) {
                case TYPE_CARD:
                    this.dataSrcDsc = '02'   // 카드
                    estMydtCdIdVal  = this.mydtCdIdVal
                    estAccIdVal     = 'card'
                    break
                case TYPE_PAY:
                    this.dataSrcDsc = '04'   // 전자금융
                    estMydtCdIdVal  = this.faceNo
                    estAccIdVal     = this.accIdVal2
                    break
            }
            const config = {
                url  : '/lc/le/02s01',
                data : {
                    mydtCusno       : this.mydtCusno,
                    infOfrmnOrgC	: this.infOfrmnOrgC,
                    mydtCdIdVal     : estMydtCdIdVal,
                    accIdVal        : estAccIdVal,
            ...
            console.log("setStlMnsXpsObtData() response===================", response)
                this.xpsEstAm  = response.estAm
                this.xpsEstDsc = response.estDsc
        ...
        // 결제내역 URL
        getDataUrl() {
            console.log("getDataUrl() start===================")
            let url = '';
            switch(this.type) {
                case TYPE_CARD:
                    url = '/lc/le/02r03'    // /lc/le/02r02 --> /lc/le/02r03
                    break
                case TYPE_PAY:
                    switch(this.subtype) {
                        case SUBTYPE_PAYMONEY:
                            url = '/lc/le/03r03'    // /lc/le/03r02
                            break
                        case SUBTYPE_PPAYCARD:
                            url = '/lc/le/04r04'    // /lc/le/04r02
                            break
                    }
                    break
            }
        ...
        getSelectComponent() {
            console.log("getSelectComponent() start===================")
            let component = '';
            switch(this.type) {
                case TYPE_CARD:
                    component = LCLE4012
        ...
        getSearchComponent() {
            console.log("getSearchComponent() start===================")
            let component = '';
            switch(this.type) {
                case TYPE_CARD:
                    component = LCLE4007
        ...
        getInfoComponent() {
            console.log("getInfoComponent() start===================")
            let component = '';
            switch(this.type) {
                case TYPE_CARD:
                    component = LCLE4112
